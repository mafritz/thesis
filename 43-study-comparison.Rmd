...
This chapter provides an overall and comparative assessment about the use and perception of the Emotion Awareness Tool (EAT) using data collected in the two previous empirical chapters of the thesis, as well as in the usability test depicted in Section \@ref(dew-ux-test). In this regard, the chapter proposes four types of assessment related either to specific features of the EAT or its perception as a whole. First, the use of the two appraisal dimensions *Valence* and *Control/Power* that has been done through the two sliders of the interface. Second, the kind and proportion of subjective feelings expressed, either as part of the underlying affective space or not. Third, the computational model linking appraisal dimensions and subjective feelings in the adopted affective space. And last, the perceived usability of the tool, which comprises the efficacy, efficiency and satisfaction in using the EAT [@brookeSUSQuickDirty1996; @lewisItemBenchmarksSystem2018; @tullisMeasuringUserExperience2013].

## Study Rationale

Taking advantage of the use of the EAT by different participants and in different conditions, the contribution aims at determining (1) to what extent participants exploited the theory-driven features of the tool, including the emotional structure provided by the underlying affective space (*i.e.*, the EATMINT circumplex adopted in both empirical contributions); and (2) whether there are observable differences in the use and perception of the tool with respect to the prototypical distance learning settings, that is, the Asynchronous and Individual setting (Asynch./Indiv.) of the previous chapter, and the Synchronous and Collaborative setting (Synch./Collab.) of Chapter \@ref(study-1) and the usability test. Naturally, the comparison between settings is not aimed at generalizing the use and perception of the EAT to the specific distance learning condition (*e.g.*, assess whether a particular feeling occurs more often when learners work individually rather than in pairs). This is clearly an overarching assessment that falls way outside the scope of the present contribution. Nonetheless, it is possible to take advantage of the two empirical studies to explore if the multipurpose vocation of the EAT can be sustained in two very different computer-mediated learning environments.

## Methods

This third empirical contribution can be considered a form of secondary data analysis [@westonRecommendationsIncreasingTransparency2019], grouping the datasets of the three empirical contributions where the EAT has been deployed. The analyses will be based over two combined datasets.

The first consists in all the emotions that have been expressed through the EAT. This dataset comprises $N_{observations} =$ `r sc.dew_combined_emotions |> nrow()` produced by $N_{participants} =$ `r sc.dew_combined_emotions$user |> unique() |> length()`. Table \@ref(tab:sc-emotions-dataset-table) illustrates how observations and participants are divided between the three datasets, with two datasets attributed to the Synch./Collab. setting, and one to the Asynch./Indiv. setting.
...
Individual attitude towards rating both dimensions together should therefore be considered in the assessment. In this regard, it is interesting to compute the individual correlation between the rating of the *Valence* dimension and the *Control/Power* dimension, and then use the average of the correlation ($M_\rho$) as in indicator of whether the two sliders tend to co-vary or not. An $M_\rho \rightarrow 0$ would suggest the two dimensions are orthogonal, whereas $M_\rho \rightarrow \pm 1$ would suggest that the two dimensions are rated exactly in the same way (positive correlation), or that one is rated asymmetrically with respect to the other. Among the $N = 74$ participants that expressed at least 2 emotions (the lower bound to comput a correlation), the average correlation observed is of $M_\rho =$ `r sc.appraisal_correlation.overall$cor |> mean(na.rm = TRUE) |> printnum()` ($SD_\rho =$ `r sc.appraisal_correlation.overall$cor |> sd(na.rm = TRUE) |> printnum()`). The correlation is greater in the Asynch./Indiv. setting, with $M_\rho =$ 0.58 ($SD_\rho$ = 0.47) compared to the Synch./Collab. setting, where $M\rho =$ 0.42 ($SD_\rho =$ 0.41). In both settings, thus, there is a highly positive correlation between the two sliders, which tend to be rated symmetrically.
