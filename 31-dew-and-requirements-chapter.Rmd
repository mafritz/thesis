...
# Building on Previous Work to Set Requirements for the Proof of Concept {#dew-chapter}
...
This chapter resumes previous work that I have conducted in my Master thesis on the subject of Emotion Awareness Tools [@fritzReinventingWheelEmotional2015] as the basis to establish a set of requirements for the proof of concept. The Master thesis was a *spin-off* of the Emotion Awareness Tools for Computer-Mediated Interactions (EATMINT) project and was mainly focused on a user experience perspective, lacking therefore most of the theoretical background set forth in Part I. The Master thesis nevertheless lead to a conceptual prototype, which I named Dynamic Emotion Wheel (DEW), with characteristics that can be extended for a more overarching tool. The prototype was first presented at the International Society for Research on Emotion (ISRE) conference [@fritzDynamicEmotionWheel2015], and later in education technology conferences [@fritzDynamicEmotionWheel2016; @fritzRealTimeEmotionalAwareness2016]. The interest elicited in both affective sciences and education technology communities corroborated further conceptual and technical work on the prototype, and is therefore an important part of why this thesis came to be. The chapter starts by briefly outlining the aim and scope of the EATMINT project as the context through which a series of initial requirements for an EAT were set forth. Second, it illustrates the Geneva Emotion Wheel [@schererGRIDMeetsWheel2013; @schererWhatAreEmotions2005], a self-report tool based on the Component Process Model depicted in Section \@ref(cpm-section). The Geneva Emotion Wheel served as inspiration to produce the core principle of the Dynamic Emotion Wheel, illustrated in the third section of the chapter. The section presents an overview of the prototype, an affective space derived from previous work in the EATMINT project, and also the main results of a usability test conducted in experimental settings. Finally, a new set of more comprehensive requirements for the proof of concept are outlined in relation with the theoretical background of Part I and limitations of the prototype.
...
```{r tf-eatmint-eat-repeated}
#| fig.align = "center", 
#| fig.cap = "(ref:tf-eatmint-eat-repeated-caption)", 
#| fig.scap = "First version of the EATMINT tool (repeated)",
#| out.width = "80%"
...
```{r gew-version-3-figure}
#| fig.align="center", 
#| fig.cap="(ref:gew-version-3-caption)",
#| fig.scap = "Third version of the Geneva Emotion Wheel, from Scherer, Shuman, et al. (2013)",
#| out.width="80%"  
include_graphics(
  here::here("figure/dew/gew-v3.png")
)
```
...
### The Geneva Emotion Wheel as Emotion Awareness Tool
...
> --- @schererSemanticStructureEmotion2019, p. 9
...
```{r dew-core-wireframe}
#| fig.align="center", 
#| fig.cap="(ref:dew-core-wireframe-caption)", 
#| fig.scap = "Core principle of the Dynamic Emotion Wheel (wireframe)",
#| out.width="60%"
knitr::include_graphics(
  here::here("./figure/dew/dew-core-principle-subset.png")
)
```
...
```{r dew-overall-commented-interface}
#| fig.align="center", 
#| fig.cap="(ref:dew-overall-commented-interface-caption)",
#| fig.scap = "Overall interface of the Dynamic Emotion Wheel", 
#| out.width="100%"
knitr::include_graphics(
  here::here("./figure/dew/dew-overall-commented-interface.png")
)
```
...
(ref:dew-expressing-sequence-caption) Interaction workflow to express an emotion through the Dynamic Emotion Wheel

```{r dew-expressing-sequence}
#| fig.align="center", 
#| fig.cap="(ref:dew-expressing-sequence-caption)",
#| fig.scap = "Interaction workflow to express an emotion through the Dynamic Emotion Wheel", 
#| out.width="32%", 
#| fig.show="hold"
...
```{r theoretical-feelings-graph}
#| fig.align='center', 
#| fig.height=4,
#| fig.width=4, 
#| fig.cap="(ref:theoretical-feelings-graph-caption)",
#| fig.scap = "The EATMINT circumplex", 
...
```{r ux-heatmap-duration}
#| fig.align="center", 
#| fig.cap="(ref:ux-heatmap-duration-caption)", 
#| fig.scap = "Heatmap of the usability test on the Dynamic Emotion Wheel",
#| out.width="80%"
knitr::include_graphics(
  here::here("./figure/dew/ux-test-heatmap.png")
)
```
...
ux_measures <- tribble(
  ...
  "Number of emotions expressed by participant", 14, ux.expressing$tot |> mean(), ux.expressing$tot |> sd(),
...
## Extending the Prototype Into a Proof of Concept by Setting New Requirements {#dew-extension-illustration}

The chapter has so far covered how the Dynamic Emotion Wheel came to be as an attempt to respond to specific requirements within the EATMINT project scope. Those requirements, though, stemmed primarily from a usability perspective and were mainly focused on the overall aim of the project, that is, enhancing collaboration in computer-mediated collaboration, particularly in dyadic interactions. In other words, they did not consider the full extent of emotional awareness as it has been illustrated in Part I of the thesis, particularly in the abstract model of Section \@ref(abstract-model-of-ea). As a consequence, in this section I set forth new and more comprehensive requirements for the proof of concept. As mentioned in the introduction, the implementation of the EAT was not a straight process, but rather iterative. Thus, some -- if not all -- of these requirements have not been defined up front, but rather calibrated and extended according to the results of the empirical contributions of Part III.

### Strengthen the Link with Appraisal Theory of Emotion

In the DEW, the computational model linking appraisal dimensions and subjective feeling was mainly driven by the need to reduce the space occupied by the displaying-expressing function of the tool. Nevertheless, as Chapter \@ref(defining-emotion-unit) has revealed, this link is at the core of what an emotion is according to appraisal theories of emotion. The importance of the cognitive evaluation of the situation can therefore be extended both technically and conceptually. For instance, even though two-dimensional models of emotion are ubiquitous, appraisal theories do not limit the number of appraisal criteria that may be useful in determining the whole gamut of emotional experiences [@moorsAppraisalTheoriesEmotion2013;@fontaineWorldEmotionsNot2007]. The tool may therefore adopt any number of appraisal dimensions that researchers or practitioners consider of help in producing or perusing emotional awareness, as well as potential new findings in fundamental emotion theory about the elicitation mechanism [@moorsTheoriesEmotionCausation2009; @scherer2022].

### Consider Emotional Awareness At All Levels of Analysis

As Chapter \@ref(eat-general-chapter) has depicted, emotional awareness may span over all the four levels of analysis identified by @keltnerSocialFunctionsEmotions1999, that is, the individual, the dyadic, the group, and the cultural levels. The DEW was primarily focused on the dyadic level. Consequently, more attention should be dedicated to (1) the individual pathway of emotional awareness as depicted in the abstract model; (2) the possibility that the tool may be used by more than two learners at the time; and (3) that cultural factors could be taken into account, for instance in determining lexicalized emotions that are known to be more frequently adopted in a determined *milieu* where the EAT is deployed, as it is the case for the EATMINT circumplex depicted above [@fritzDynamicEmotionWheel2016; @reisAffectiveStatesCSCL2015; @ogarkovaFolkEmotionConcepts2013].

### Cover Synchronous and Asynchronous Learning Situations

Computer-mediated learning environments can be adopted in a wide variety of situations. One recurrent and overarching distinction that is often made in education technology consists in synchronous and asynchronous learning settings. The DEW was primarily thought for synchronous settings, were learners collectively used the tool during a task to be completed in a defined and seamless period of time [@molinariEmotionFeedbackComputermediated2013; @avrySharingEmotionsContributes2020]. As illustrated in Chapter \@ref(general-affective-background), though, there is a growing consensus in endowing all form of computer-mediated learning environments with the affective dimension [@lavoueEmotionalDataCollection2017; @lavoueEmotionAwarenessTools2020; @jezegouCreerPresenceDistance2010]. In particular, students may be in particular need of a socio-affective presence precisely when the learning settings limit contacts with the colleagues, such as when learning happens only at the individual level and at different timescales. The proof of concept should therefore adapt to different temporal dynamics, especially with respect to the monitoring-perceiving function of awareness.

### Provide Researchers and Practitioners with the Possibility to Adapt the Tool to Their Needs

Endowing a computer-mediated learning environment with emotional awareness requires technical skills that are often beyond reach for researchers and practitioners without a computer science background. This certainly represents an hindrance to the possibility of taking full advantage of the potential diversity of use that could be made of an EAT, or research questions that could be investigated if emotional awareness were provided more easily and without the need of dedicated hardware or software [@cerneaSurveyTechnologiesRise2015; @feidakisReviewEmotionAwareSystems2016].

The proof of concept should thus attempt to make the EAT readily available to other scholars and practitioners interested in an Emotion Awareness Tool with some prototypical characteristics that are necessarily fixed, but others that can be configured to a greater extent. The configuration of the tool is also intended to play a prominent role in research, since different configurations may be linked to different theoretical or pedagogical objectives. Setting up different instances of the tool may therefore become a type of intervention, whose effects can be traced back to phenomena of interest [@buderGroupAwarenessTools2011; @buderGroupAwareness2021].

### Foster Open Science Principles

Finally, congruently with the thesis perspective depicted in the introduction, the proof of concept aims at fostering the adoption of Open Science principles [@Nosek1422; @cruwellSevenEasySteps2019; @chambersSevenDeadlySins2017], namely in terms of:

-   Transparency and consistency in the inner functioning of the tool;

-   Facilitating comparison of studies that have adopted the EAT, even though in different configurations;

-   Easing reproducibility and replicability of the data obtained through the tool;

-   Recognition and credit for scholars contributing to the advancement of the tool, for instance by creating or adapting underlying affective spaces from other tools, empirical contributions, or fundamental theories.

## Summary

This chapter provided an overview of previous work conducted on a prototype of an EAT, named Dynamic Emotion Wheel, that the proof of concept will extend according to a new set of comprehensive requirements. These requirements cover theoretical, technical, and pedagogical objectives, which must coalesce in a relatively simple and parsimonious tool, requiring minimal dedicated hardware and software, as well as technical know-how to be adopted by scholars and practitioners alike. The next two chapters in this Part II illustrate how the requirements have been attempted to be met mainly by two intertwined elements. First, a better defined and extended computational model linking appraisal criteria and subjective feeling, thoroughly depicted in Chapter \@ref(computational-model). Second, a toolbox illustrated in Chapter \@ref(toolbox-chapter), which is built around this core computational model and allows scholars and practitioners to set up and configure their own instance of an EAT, as well as exporting data gathered through the tool.
