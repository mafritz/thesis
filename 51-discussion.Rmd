...
One of the main advantages of SCM is that it is possible to derive, according to the specific effect that we want to test in the model, what other variables should be controlled/conditioned for depending on the type of effect we are interested in. For instance, in the general example, we may be interested in the direct effect of $X$ on $Y$. In this case, the structure of the DAG implies that we must control for variables $A$, $F$, and $M$, but not for the others. The control of $B$ is optional, depending on the degree of accuracy in estimating the effect $X \rightarrow Y$. Conversely, if we were interested in the total effect of $X$ on $Y$, then we must control only for $A$ and $F$, but not for $M$ and the others, with $B$ once again as optional. This is particularly relevant in the study of emotional awareness in computer-mediated learning environment, since there are many potential variables at stake and controlling all of them would not only be impossible, but also potentially induce spurious correlations. In other words, what variable are controlled for depend on the structural causal model itself.
...
"EAT Features" [pos="2.900,-3.260"]
"EAT Use" [pos="-1.379,-0.537"]
"Emotional Awareness" [exposure,pos="-2.627,0.539"]
"Emotional Competence" [pos="-2.584,-3.207"]
"Learning process or outcome" [outcome,pos="3.027,0.512"]
Appraisal [pos="-2.601,-1.303"]
Partners [pos="0.973,-3.225"]
Regulation [pos="0.132,0.548"]
Task [pos="-0.929,-3.216"]
...
Partners -> Regulation
Partners -> Task
Regulation -> "Learning process or outcome"
Task -> "EAT Use"
Task -> "Learning process or outcome"
Task -> Appraisal
Task -> Regulation
}')
...
-   There are four high-level variables that play a prominent role, which are depicted on the top of the model: The EAT features, learners' Emotional Competence, the characteristics of the Task, and the presence/characteristics of the Partner(s). Among the four high-level variables, I posit that there is an influence of the Partner(s) on the Task at hand and the EAT Features, since the presence of Partners determine what learners will do, but also how the EAT would look like (e.g. configuration according to intra-personal, inter-personal, or both form of awareness).

-   Congruently with the appraisal-driven perspective of the thesis, I assume that the Appraisal variable -- that is, how the situation at hand is evaluated -- play a prominent role in the model. Appraisal is influenced by all the four high-level variables depicted above. Appraisal is a direct ascendant of Emotional Awareness, since I venture that a great deal of variation in Emotional Awareness is determined on how the situation is evaluated. The effect of Appraisal on Emotional Awareness can be direct or mediated by EAT Use. In fact, it is possible to construe Emotional Awareness even without using the EAT, for instance by inferring emotional meaning from cues outside the emotional information provided by the EAT.

-   The EAT Use also plays a prominent role in the model, since it directly influences Emotional Awareness. At the same time, the use of the EAT is influenced by the four high-level variables.

-   Emotional Awareness directly influence the Regulation strategy adopted (e.g. Self-Regulation, Co-Regulation or Socially-Shared Regulation). The Regulation, at the same time, also depends on three out of four high-level variables, since I venture that the EAT Features do not have a direct influence on Regulation. The influence of the EAT on Regulation is completely mediated by the use.

-   Finally, Learning Processes or Outcomes are directly influenced by the Regulation, as well as by the Task, since, for instance, if the Task is too difficult, there will not be any Learning Process or Outcome regardless of all other variables implied.

The structure of the DAG implies that, in spite of the multiple associations between variables, for measuring the direct effect of Emotional Awareness on Learning Process and Outcomes, it is necessary to control only for the Task and Regulation. The Task is fairly obvious, since to measure the effect of different degrees of Emotional Awareness the Task should be hold constant. The Regulation, on the other hand, cannot be hold constant. So an alternative could be to measure learner's regulatory abilities, for instance with the regulation and management dimensions of the Geneva Emotion Competence Test adopted in the longitudinal study.
