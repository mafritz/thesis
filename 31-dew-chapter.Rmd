...
# Previous Work on a Prototype of an EAT: the Dynamic Emotion Wheel {#dew-chapter}
...
Another shortcoming of the GEW is its potential to fulfill the perceiving-monitoring function of an awareness tool. The size of the interface leaves little space to provide a dedicated space for contemplating the emotion expressed through the tool, especially from a longitudinal perspective. As in @tranEmotionsDecisionMakingProcesses2012, it would be possible to mark directly on the circles the expressed emotions with some form of sequential encoding to emulate time, but this mecanisms may easily overcrowd the interface after a few responses. One may duplicate the interface, for instance leaving the upper GEW for expressing-displaying, and the bottom GEW for perceiving-monitoring; but the tool would in this case occupy twice the already considerable size of one GEW alone.

To sum up, I considered the opportunity to take advantage of GEW by maintaining its many theoretical and empirically validated features, but at the same time orient them towards an EAT that could meet the requirements set forth with the EATMINT project's member outlined in Section \@ref(requirements-for-a-second-version-of-an-eat). The result is the core principle of the Dynamic Emotion Wheel described in the next section.
