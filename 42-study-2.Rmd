...
### Emotional Competence as an Intervening Factor
...
### Measures About the Adoption and Use of the EAT
...
As one may expect from the small sample size at hand, all the parameters remain uncertain and the test provide inconclusive results about the possibility that the perceived emotional awareness usefulness can be determined by emotional sub-competences. Is is nevertheless interesting to note that, assuming the parameters would hold in a larger sample, emotion recognition has virtually no effect, whereas regulation and management have effect that is respectively almost twofold, and over twofold that of understanding. Emotion recognition was performed through efferent manifestations that would normally be available in face-to-face interactions, and its lack of effect could be considered as supporting the need for an EAT to represent emotion with a structure rather than efferent cues. The importance of regulatory processes would also be corroborated, with an interesting perspective coming from the greater effect of inter-personal emotion regulation. These are nevertheless only wild speculations for the time being. The only relevant element to retain may therefore be that the GECo test and its sub-competences are viable and potentially useful measures to further investigate this kind of effects.
...
Data basically show that participants tended to rate a similar frequency for their colleagues based on their own frequency, a phenomenon already encountered in other contributions [@eligioEmotionUnderstandingPerformance2012; @molinariEmotionFeedbackComputermediated2013; @molinariFeedbackEmotionnelCollaboration2013]. When participants lack the knowledge of their partners emotions, they use themselves as the *measuring stick*. The frequency is also pretty much independent from the *observed* frequency. This is particularly relevant when the *observed* frequency is compared with the *class* frequency, since the graphical representation of the whole class was overtly aimed at the relative occurrence of subjective feelings. So, if the word cloud about the class as a whole was retained in memory, that at least this measure could potentially overlap. The fact that is far from being the case seems to suggest that, simply, there has been no recollection of the emotional experience from the EAT.
...
### Scarce Adoption and Use of the EAT Overall
...
### Perceived Usefulness Drops with Concrete Use
...
### No Recollection of the Emotional Experience Whatsoever
